      ...
      <h4 style="font-size: 20px;" class="media-body pb-3 mb-0 small lh-125 border-gray">
        <strong class="d-block text-gray-dark"><?php echo $uname; ?> - 
        	<?php 
        	   ...
        	   $mainf1 = mysqli_query($conn,"SELECT * FROM $followers WHERE user_id = '$id_uname'");
		$tot = 0;
        ...
        $mainf = mysqli_query($conn,"SELECT * FROM $followers WHERE user_id = '$id_uname'");

        if(mysqli_fetch_assoc($mainf)){
        $mainf2 = mysqli_query($conn,"SELECT * FROM $followers WHERE user_id = '$id_uname' ORDER BY id DESC");

	        while($foll = mysqli_fetch_assoc($mainf2)){
	        	$foreign = $foll['following'];
